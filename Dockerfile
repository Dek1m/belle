...
RUN apt-get update \
    && apt-get install -y --no-install-recommends git curl ca-certificates passwd bash coreutils ncurses-base \
...
RUN echo "$CACHEBUST" \
    ...
    && git clone --depth 1 https://github.com/Dek1m/mia-term.git /app/mia/modules/term
